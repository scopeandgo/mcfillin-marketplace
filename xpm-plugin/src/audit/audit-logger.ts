 ...
 *
 * Log location:
 * - Mac: /Library/Application Support/ClaudeMCP/xpm-mcp/audit.log
 * - Windows: C:\ProgramData\ClaudeMCP\xpm-mcp\audit.log
 */
class AuditLoggerImpl {
  ...
  ensureLogDirectory(): void {
    try {
      ...
      console.error(
        `[mcfillin-xpm] Warning: Cannot create audit log directory "${AUDIT_LOG_DIR}": ${error.message}`,
      );
      console.error(
        `[mcfillin-xpm] Ensure the directory has been created by your system administrator with appropriate write permissions.`,
      );
    }
  }
  ...
  logToolCall(
    ...
    try {
      ...
      const error = err as Error;
      console.error(
        `[mcfillin-xpm] Warning: Failed to write audit log: ${error.message}`,
      );
    }
  }
}
